...
def get_next_sequence_value(sequence: list[int]) -> int:
    ...
    difference_sequences[-1].append(0)
    for i in range(len(difference_sequences) - 2, -1, -1):
        difference_sequences[i].append(difference_sequences[i][-1] + difference_sequences[i + 1][-1])

    return sequence[-1] + difference_sequences[0][-1]
